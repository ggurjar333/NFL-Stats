...
from src.sportsradar.extract.primaryfeeds import PrimaryFeeds
...
class TestPrimaryFeeds(unittest.TestCase):
    ...
    def test_get_current_season_schedule(self):
        result = self.primary_feeds.get_current_season_schedule(access_level=TestConstants.ACCESS_LEVEL,
    ...
    def test_get_current_week_schedule(self):
        result = self.primary_feeds.get_current_week_schedule(access_level=TestConstants.ACCESS_LEVEL,
                                                               language_code=TestConstants.LANGUAGE_CODE,
                                                               version=TestConstants.VERSION,
                                                               file_format=TestConstants.FORMAT,
                                                               api_key=TestConstants.API_KEY)
        ...
        assert result.status_code == self.expected_status, f"Expected Status code {self.expected_status}, but got{results.status_code}."

    def test_get_season_schedule(self):
        for year_ in self.year:
            for season in self.season_type:
                result = self.primary_feeds.get_seasons_schedule(access_level=TestConstants.ACCESS_LEVEL,
                                                                  language_code=TestConstants.LANGUAGE_CODE,
                                                                  version=TestConstants.VERSION,
                                                                  year=year_,
                                                                  season_type=season,
    ...
    def test_get_week_schedule(self):
        result = self.primary_feeds.get_weekly_schedule(access_level=TestConstants.ACCESS_LEVEL,
                                                         language_code=TestConstants.LANGUAGE_CODE,
                                                         version=TestConstants.VERSION,
