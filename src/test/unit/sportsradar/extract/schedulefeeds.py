...
from src.sportsradar.extract.schedulefeeds import ScheduleFeeds
...
class TestScheduleFeeds(unittest.TestCase):
    ...
    def test_get_current_week_schedule(self):
        result = self.schedule_feeds.get_current_week_schedule(access_level = TestConstants.ACCESS_LEVEL,
                                                                    language_code= TestConstants.LANGUAGE_CODE,
                                                                    version= TestConstants.VERSION,
                                                                    file_format=TestConstants.FORMAT,
                                                                    api_key = TestConstants.API_KEY )
    ...
    def test_get_season_schedule(self):
        for year_ in self.year:
            for season in self.season_type:
                result = self.schedule_feeds.get_seasons_schedule(access_level = TestConstants.ACCESS_LEVEL,
                                                            language_code = TestConstants.LANGUAGE_CODE,
                                                            version = TestConstants.VERSION,
                                                            year = year_,
                                                            season_type = season,
    ...
    def test_get_week_schedule(self):
        result = self.schedule_feeds.get_weekly_schedule(access_level = TestConstants.ACCESS_LEVEL,
                                                    language_code=TestConstants.LANGUAGE_CODE,
                                                    version=TestConstants.VERSION,
...
if __name__=='__main__':
    unittest.main(argv = [''], defaultTest = 'TestScheduleFeeds')